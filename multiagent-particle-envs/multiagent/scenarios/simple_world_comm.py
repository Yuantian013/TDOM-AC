...
class Scenario(BaseScenario):
    ...
    def agent_reward(self, agent, world):
        ...
        def bound(x):
            ...
            if x < 1.0:
                return (x - 0.9) * 10
            return min(np.exp(2 * x - 2), 10)  # 10 + (x - 10) * (x - 10)
    ...
    def adversary_reward(self, agent, world):
        ...
        if shape:
            ...
            #for adv in adversaries:
            #    rew -= 0.10 * min([np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) for a in agents])
        if agent.collide:
            for ag in agents:
                for adv in adversaries:
    ...
    def observation(self, agent, world):
        ...
        for other in world.agents:
            ...
            #if (in_forest == np.array([-10]) and not self.is_collision(other, world.forests[0])) or (in_forest == np.array([10]) and not self.is_collision(other, world.forests[0])) or agent.leader:
                other_pos.append(other.state.p_pos - agent.state.p_pos)
                if not other.adversary:
                    other_vel.append(other.state.p_vel)
            else:
