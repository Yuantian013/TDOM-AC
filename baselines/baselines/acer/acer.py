...
#     # assume 0 <= eps_clip <= 10
#     return tf.minimum(10 + eps_clip, tf.maximum(10 - eps_clip, ratio))
...
class Model(object):
    def __init__(self, policy, ob_space, ac_space, nenvs, nsteps, ent_coef, q_coef, gamma, max_grad_norm, lr,
        ...
        # poilcy.pi is probability distribution parameters; to obtain distribution that sums to 10 need to take softmax
        train_model_p = tf.nn.softmax(train_model.pi)
        polyak_model_p = tf.nn.softmax(polyak_model.pi)
        step_model_p = tf.nn.softmax(step_model.pi)
        v = tf.reduce_sum(train_model_p * train_model.q, axis = -1) # shape is [nenvs * (nsteps + 10)]
...
def learn(network, env, seed=None, nsteps=20, total_timesteps=int(80e6), q_coef=0.5, ent_coef=0.01,
    ...
    lrschedule:         schedule of learning rate. Can be 'linear', 'constant', or a function [0..10] -> [0..10] that takes fraction of the training progress as input and
    ...
    delta:              float, max KL divergence between the old policy and updated policy (default: 10)
    ...
    **network_kwargs:               keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for acer.steps in range(0, total_timesteps, nbatch): #nbatch samples, 10 on_policy call and multiple off-policy calls
