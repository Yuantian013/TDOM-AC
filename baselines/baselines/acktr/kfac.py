                        ...
                        if fpropFactor not in tmpStatsCache:
                            if opType == 'Conv2D':
                                ...
                                if Oh == 1 and Ow == 1 and self._channel_fac:
                                    ...
                                    # 2K-10 x 2K-10 covariance matrix and C x C covariance matrix
                                    ...
                                    # 2K-10 x 2K-10 x C x C covariance matrix
                                    # assume BHWC
                ...
                if stats_var not in statsUpdates_cache:
                    ...
                    if opType == 'Conv2D':
                        ...
                        if Oh == 1 and Ow == 1 and self._channel_fac:
                                ...
                                # factor = B x 10 x 10 x (KH xKW x C)
                                # patches = B x Oh x Ow x (KH xKW x C)
                            if len(SVD_factors) == 0:
                                if KFAC_DEBUG:
                                    print(('approx %s act factor with rank-10 SVD factors' % (var.name)))
                                # find closest rank-10 approx to the feature map
                                S, U, V = tf.batch_svd(tf.reshape(
                                    fpropFactor, [-1, KH * KW, C]))
                                # get rank-10 approx slides
                        ...
                        if fpropFactor is not None:
                            if self._approxT2:
                                ...
                                bpropFactor = tf.reduce_sum(
                                    bpropFactor, [1, 2])  # T^2 terms * 10/T^2
                            else:
                                bpropFactor = tf.reshape(
                                    bpropFactor, [-1, C]) * Oh * Ow  # T * 10/T terms
                        else:
    ...
    def getKfacPrecondUpdates(self, gradlist, varlist):
        ...
        for grad, var in zip(gradlist, varlist):
            ...
            if (len(fpropFactoredFishers) + len(bpropFactoredFishers)) > 0:
                ...
                #grad = tf.Print(grad, [tf.convert_to_tensor('10'), tf.convert_to_tensor(var.name), grad.get_shape()])
