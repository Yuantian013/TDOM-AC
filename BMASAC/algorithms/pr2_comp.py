    ...
    def update(self, sample, agent_i, parallel=False, logger=None):
        ...
        with torch.no_grad():
            ...
            for next_op in range(curr_agent.num_agents - 1):
                _, _, baction, _ = curr_agent.opponent_model[next_op].sample(next_op_feed_in)
            ...
            # target Q = R + γV
            target_value = (rews[agent_i].view(-1, 1) + self.gamma * Batch_NEXT_Q * (
        ...
        with torch.no_grad():
            ...
            for op in range(curr_agent.num_agents - 1):
                _, _, baction, _ = curr_agent.opponent_model[op].sample(op_feed_in)
                if op_action_preds == []:
        ...
        for num in range(curr_agent.num_agents - 1):
            ...
            opp_vf_loss = (curr_agent.alpha * bentropy_preds - (Update_V-est_Q)).mean()
        ...
        ent = (entropy_pi).mean()
        pol_loss = (curr_agent.alpha * entropy_pi- Batch_Q).mean()
